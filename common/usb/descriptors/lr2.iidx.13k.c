#include "usb/descriptors/lr2.iidx.13k.h"

// When differing, original values are commented to the right
const USB_DEVICE USB_ATTRIBUTES LR2_Device = {
  ...
  .idProduct        = 0x8047,
  .bcdDevice        = 0x0101,
  .iManufacturer    = StringID_Vendor_LR2,            // 1
  .iProduct         = StringID_Product_LR2,           // 2
  .iSerialNumber    = StringID_Serial,                        // 0
  .bNumConfigurations = 1
};

const USB_CONFIG USB_ATTRIBUTES LR2_Configuration = {
  .Config = {
  ...
  .HID_Report = {
    ...
    .wDescriptorLength      = sizeof(LR2_Report)      // 50
  },
