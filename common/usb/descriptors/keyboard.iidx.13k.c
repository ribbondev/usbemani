#include "usb/descriptors/keyboard.iidx.13k.h"

// When differing, original values are commented to the right
const USB_DEVICE USB_ATTRIBUTES Keyboard_Device = {
  ...
  .idProduct        = 0x7ea1,
  .bcdDevice        = 0x0500,
  .iManufacturer    = StringID_Vendor_Keyboard,               // 1
  .iProduct         = StringID_Product_Keyboard,              // 2
  .iSerialNumber    = StringID_Serial,                        // 0
  .bNumConfigurations = 1
};

const USB_CONFIG USB_ATTRIBUTES Keyboard_Configuration = {
  .Config = {
    ...
    .bMaxPower              = USB_CONFIG_HEADER_MAX_POWER,    // 500mA
  },

  .HID_Interface = {
    .bLength                = sizeof(USB_INTERFACE),          // 9
    .bDescriptorType        = USB_INTERFACE_TYPE,             // 0x04
    .bInterfaceNumber       = InterfaceID_Joystick,           // 1
    .bAlternateSetting      = 0,
    .bNumEndpoints          = 2,                              // 1
  ...
  .HID_Report = {
    ...
    .wDescriptorLength      = sizeof(Keyboard_Report)         // 50
  },
