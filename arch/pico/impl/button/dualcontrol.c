...
static bool down[ANALOG_CHANNELS_AVAILABLE] = { false };

void _impl_button_poll(void) {
  for (int i = 0; i < BUTTONS_ACTIVE; i++) {
    // If we're using DUALCONTROL, expect that last 4 buttons are conventional microswitches (last 4 of 11)
    if (i >= ANALOG_CHANNELS_AVAILABLE ) {
